%{
Ripple Sample Data Walkthrough v2
...
fileName = 'sample_data_set.nev'; % default location for sample data is ../Users/../Trellis/sampleData

[ns_RESULT, hFile] = ns_OpenFile(fileName);
[ns_RESULT, nsFileInfo] = ns_GetFileInfo(hFile);

numEnt = length(hFile.Entity);
switch(fileName) % choose which entities from the 'hFile' struct you want to plot
    case 'sample_data_set.nev'
        ent_to_plot = [1 5 33:36 40 68 71 72 75]; % only plot a subset of entities
    otherwise
        ent_to_plot = 1:numEnt; % plot all of the entities
end

fs_clock = 30000;

% Loop through entities, gather data, and plot:
for ii = ent_to_plot
