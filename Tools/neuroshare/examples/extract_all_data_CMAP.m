...
date = '09212017';
nerve = 'L_tibial';
dataType = 'Stim'; % dataType can be:
                    % Neural - 'Spike','LFP', 'Hi-Res', or 'Raw'; 
                    % Stim - 'Stim'
                    % Analog I/O - 'Analog 30k' or 'Analog 1k'
                    % Digital I/O - 'Digital'
                    
dataChannel = 5 ;   % A-1 starts at 1, A-2 starts at 33, 
                   % B-1 starts at 129
                   % Analog I/O starts at 10241
                   % Digital is 1 to 4 or 'parallel'
                   
plotStatus = false; % Whether or not to plot the data
% savepath = '\\SSOE-BIOE-FS5\CuiShared\USERS\xiz149\Documents\MATLAB\sally_TrellisDoc\exports';
% amp = [10,40,50,60,80,120,140,160,180,240,260];
%%
% amp = [20,25,30,40,50,60,80,100,200,300,500,750,1000];
% amp = [3 5 10 20 25 30 40 50 60 100 200 300 500 750 1000];
amp =  [ 3 5 7 8 9 10 15 20 25 30 40 50 60 80 100 ];

for I=1:length(amp);
% for i = 1:5
% completeFilePath = ['\\SSOE-BIOE-FS5\CuiShared\USERS\xiz149\Documents\MATLAB\sally_TrellisDoc\06212017\Right_Peroneal_',num2str(amp(I)),'uA','.nf3'];
% completeFilePath = ['\\SSOE-BIOE-FS5\CuiShared\USERS\xiz149\Documents\MATLAB\sally_TrellisDoc\CMAP_experiment_1\datafile_CMAP_tibstim_gastrocrecord_',num2str(amp(I)),'.nf3'];
% completeFilePath =['\\SSOE-BIOE-FS5\CuiShared\USERS\xiz149\Documents\MATLAB\sally_TrellisDoc\08012017_cmap\Left_peroneal_',num2str(amp(I)),'uA','.nf3'];
completeFilePath = ['\\SSOE-BIOE-FS5\CuiShared\USERS\xiz149\Documents\MATLAB\09212017\Left_tibial_',num2str(amp(I)),'ua.nf3']
...
% mkdir(savepath)
save([animalID,'_',date,'_',num2str(dataChannel),'_',nerve,'_',dataType,'_',num2str(amp(I)),'.mat']);
end
